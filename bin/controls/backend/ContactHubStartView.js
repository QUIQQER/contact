...
define('package/quiqqer/contact/bin/controls/backend/ContactHubStartView', [
    ...
    return new Class({
        ...
        Extends: QUIControl,
        Type: 'package/quiqqer/contact/bin/controls/backend/ContactHubStartView',
        ...
        $onImport: function () {
            ...
            if (Scope) {
                Scope.querySelectorAll('[data-name="contactHubPlaceholder"]').forEach((field) => {
                    field.placeholder = Locale.get(
                        'quiqqer/contact',
                        'brick.control.contactHub.setting.' + field.name + '.placeholder'
                    );
                });
            ...
            QUIAjax.get('package_quiqqer_contact_ajax_contactHub_isAiAgentViewAvailable', (available) => {
                if (available) {
                    return;
                }
        ...
        $groupSettings: function (scope) {
            [['startView', 'start'], ['formEnabled', 'offers'], ['predefinedButtons', 'buttons'], ['btnStyle', 'appearance']].forEach(([name, section]) => {
                ...
                cell.colSpan = 2;
                cell.textContent = Locale.get('quiqqer/contact', 'contact.contactHub.section.' + section);
        ...
        $initActionHint: function (scope) {
            ...
            hint.textContent = Locale.get('quiqqer/contact', 'contact.contactHub.noActions');
            hint.hidden = true;
            ...
            const update = () => {
                ...
                let channels = [];
                try { channels = JSON.parse(field('predefinedButtons')?.value || '[]'); } catch (e) { /* Empty configuration. */ }
                const predefined = Array.isArray(channels) && channels.some((channel) => {
                    if (!channel || [false, 0, '0'].includes(channel.active)) {
                        return false;
                    }
                ...
                if (formEnabled || custom || predefined) {
                    return;
                }
